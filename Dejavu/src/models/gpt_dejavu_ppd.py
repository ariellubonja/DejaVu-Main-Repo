# Copyright (c) 2024, Tri Dao, Ariel Lubonja
...
def remap_state_dict_gpt2(state_dict, config):
    """Remaps the keys and adjusts the tensors in the state_dict of a GPT-2 model to be compatible with a custom model architecture.

    This function modifies the keys in the state dictionary to fit the expected naming and structure used in a custom
    transformer model. It also adjusts tensor dimensions to accommodate differences in expected input shapes, particularly
    with embeddings and layer normalization parameters.

    Args:
        state_dict (OrderedDict): The original state dictionary (weights, biases, batch-norm params etc.) from a pretrained GPT-2 model.
        config (GPT2Config): Configuration object that contains attributes like vocab_size, pad_vocab_size_multiple,
                             num_hidden_layers, etc., used to adjust the tensor shapes and remap keys.

    Returns:
        OrderedDict: A new state dictionary with remapped keys and adjusted tensor dimensions to be loaded into a new model architecture.
        Should have no effectual change on state_dict, just a renaming of the keys

    Raises:
        KeyError: If expected keys are not found in the original state_dict, indicating it might not be a valid GPT-2 model state_dict.
    """
    # Word embedding and position embedding. Remap position embeddings
    def key_mapping_pos_emb(key):
        return re.sub(r"^wpe.", "transformer.embeddings.position_embeddings.", key)

    # Apply position embedding key mapping to state_dict
    ...
    def key_mapping_ln(key):
        ...
        return key
    # Apply layerNorm key mapping to state_dict
    ...
    for d in range(config.num_hidden_layers):
        ...
        # TODO Ariel I think this function just does some compatibility mapping across
        # TODO Ariel the different classes they use. May need to look elsewhere to implement PPD
...
def shard_state_dict_tp(state_dict, config, world_size, rank):
    ...
    Args:
        state_dict (OrderedDict): The state dictionary of the model to be sharded.
        config (GPT2Config): Configuration object containing attributes like vocab_size.
        world_size (int): Total number of GPUs that will be used in parallel.
        rank (int): ?

    Returns:
        OrderedDict: A new state dictionary with parameters appropriately sharded.
    """
    pad_vocab_size_multiple = getattr(config, "pad_vocab_size_multiple", 1)
    vocab_size = (
        math.ceil(config.vocab_size / pad_vocab_size_multiple) * pad_vocab_size_multiple    )
    ...
    # Ariel: These two functions look identical, save for the shape[i]
    # TODO Ariel I still don't understand what these do
...
def create_mixer_cls(config, layer_idx=None, process_group=None, device=None, dtype=None):
    # Ariel: This function seems to decompose the config into its
    # components, and set the corresponding flags
    # Ariel: Added comments with GPT-4. Don't know if they're correct
    ...
    # Check for depthwise convolution in attention and assert if it's used in a parallel setting
    ...
    # if not parallel (serial), do simple Multi-Head Attention (MHA)
    if process_group is None:
        mha_cls = MHA
    else:
        # if sparse MLP&ATT, initiate sparsity MHA,
        # else initialize original MHA
        # Ariel: if stmt. below says OR, but comment above says AND
    ...
    # LOWPRIORITY Ariel What is a mixer class? Attention?
    mixer_cls = partial(
        mha_cls,
